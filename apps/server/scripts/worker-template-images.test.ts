...
const productionCu128 = "somecatco/kastard-worker-cu128:0.1.0-build.7";
const productionCu130 = "somecatco/kastard-worker-cu130:0.1.0-build.7";

describe("Worker template images", () => {
	test("selects Production and Beta template identities", () => {
		expect(workerTemplateIdEnvironmentName("RUNPOD", "cu128", "production")).toBe(
			"RUNPOD_WORKER_TEMPLATE_ID_CU128",
		);
		expect(workerTemplateIdEnvironmentName("RUNPOD", "cu130", "beta")).toBe(
			"RUNPOD_WORKER_TEMPLATE_ID_BETA_CU130",
		);
		expect(workerTemplateIdEnvironmentName("VAST", "cu128", "beta")).toBe(
			"VAST_WORKER_TEMPLATE_ID_BETA_CU128",
		);
		expect(workerTemplateName("cu128", "production")).toBe("kastard-worker-cu128");
		expect(workerTemplateName("cu130", "beta")).toBe("kastard-worker-beta-cu130");
	});
